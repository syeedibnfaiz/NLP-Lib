 ...
 */
public class RelexPipeAIMed2 extends Pipe {
    ...
    public Instance pipe(Instance carrier) {
        ...
        SimpleDepGraph depGraphCC = instance.ccDepGraph;
        ...
        pl = addFormTypeFeatures(pl, s, depGraph, depGraphCC, entity1, entity2);
        pl = addBaselineFeatures(pl, s, depGraph, depGraphCC, entity1, entity2);
        ...
        pl = addRule3Features(pl, s, depGraph, depGraphCC, entity1, entity2);
    ...
    PropertyList addRule1Features(PropertyList pl, Sentence s, SimpleDepGraph depGraph, SimpleDepGraph depGraphCC, int entity1, int entity2) {
        int ruleNum = RULE_EXTRACTOR.check(s, depGraph, depGraphCC, entity1, entity2);
        ...
        pl = PropertyList.add(subjFeature + ruleNum, 1.0, pl);
        //pl = PropertyList.add(partmodFeature, 1.0, pl);
        pl = PropertyList.add(rcmodFeature + ruleNum, 1.0, pl);
        ...
        for (int i = 0; i < pathList.size(); i++) {            
            ...
            //if (relation.matches("-?(nsubj.*|rcmod|partmod|dobj|appos|conj|dep|xcomp)") /*&& !relation.equals(last)*/) {
                ...
                if (relation.matches("-?(nsubj|nsubjpass|partmod|rcmod)")) {                    
                    ...
                    collapsedPath.append(relation).append(":");
                    //collapsedPath.append("other").append(":");
                }                
                last = relation;
            }
    ...
    PropertyList addBaselineFeatures(PropertyList pl, Sentence s, SimpleDepGraph depGraph, SimpleDepGraph depGraphCC, int entity1, int entity2) {
        //int ruleNum = RULE_EXTRACTOR.check(s, depGraph, depGraphCC, entity1, entity2);
    ...
    PropertyList addFormTypeFeatures(PropertyList pl, Sentence s, SimpleDepGraph depGraph, SimpleDepGraph depGraphCC, int entity1, int entity2) {
        int form = RULE_EXTRACTOR.check(s, depGraph, depGraphCC, entity1, entity2);
        pl = PropertyList.add("FORM=" + String.valueOf(form), 1.0, pl);
        return pl;
    }
    PropertyList addRule3Features(PropertyList pl, Sentence s, SimpleDepGraph depGraph, SimpleDepGraph depGraphCC, int entity1, int entity2) {
        int ruleNum = RULE_EXTRACTOR.check(s, depGraph, depGraphCC, entity1, entity2);
        List<String> pathList = depGraphCC.getPathAsList(entity1, entity2, false);
        ...
        if (pathList != null) {
            for (int k = 1; k < pathList.size(); k++) {
                if (pathList.get(k).matches("-?prep_between") && pathList.get(k - 1).matches("-?prep_between")) {                    
                    pl = PropertyList.add("PREP_BETWEEN=TRUE", 1.0, pl);
                    break;
                } else if (pathList.get(k).matches("-?prep.*") && pathList.get(k - 1).matches("-?prep.*")) {
                    pl = PropertyList.add("PREP_PREP=TRUE", 1.0, pl);
                    break;
                }
            }
            ...
            pl = PropertyList.add("PREP_PATH=" + prepPath, 1.0, pl);
        }
        return pl;
    }
    ...
    PropertyList addSurfaceFeatures(PropertyList pl, Sentence s, SimpleDepGraph depGraph, int entity1, int entity2) {                
        ...
        boolean leftDomain = false;
        String sentLeft = s.toString(Math.max(0, entity1-5), entity1);
        for (int i = 0; i < restrictionTerms.length; i++) {
            if (sentLeft.contains(restrictionTerms[i])) {
                pl = PropertyList.add("LDOMAIN=" + restrictionTerms[i], 1.0, pl);
                leftDomain = true;
                break;
            }
        }        
