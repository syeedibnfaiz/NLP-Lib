 ...
 */
public class Arg1ConnInstance2FeatureVector extends Pipe {
    ...
    public Arg1ConnInstance2FeatureVector() {
        super(new Alphabet(), new LabelAlphabet());
    }
    
    @Override
    public Instance pipe(Instance carrier) {
        Arg1ConnInstance instance = (Arg1ConnInstance) carrier;
        Tree root;
        try {
        ...
        int arg1HeadPos = instance.getArg1HeadPos();
        int arg2HeadPos = instance.getArg2HeadPos();
        
        PropertyList pl = null;
        pl = addBaselineFeatures(pl, root, connStart, connEnd, arg1HeadPos, arg2HeadPos);
    ...
    private PropertyList addBaselineFeatures(PropertyList pl, Tree root, int connStart, int connEnd, int arg1HeadPos, int arg2HeadPos) {        
        ...
        pl = PropertyList.add("E="+leaves.get(arg1HeadPos).value(), 1.0, pl);
        Tree headParent = leaves.get(arg1HeadPos).parent(root);
        pl = PropertyList.add("Conn&E-type="+conn.toLowerCase()+"&"+headParent.value()/*.substring(0, 2)*/, 1.0, pl);
        
        //F-head after or before connective
        if (arg1HeadPos < connStart) {
            pl = PropertyList.add("F="+"before", 1.0, pl);
            pl = PropertyList.add("CAT&F="+category+"&"+"before", 1.0, pl);
        }
        else {
            pl = PropertyList.add("F="+"after", 1.0, pl);
            pl = PropertyList.add("CAT&F="+category+"&"+"after", 1.0, pl);
        }
        
        //arg1Head before/after arg2Headpos
        if (arg1HeadPos < arg2HeadPos) {
            pl = PropertyList.add("ARG1<ARG2", 1.0, pl);
            //pl = PropertyList.add("ARG1<ARG2&"+category, 1.0, pl);
        } else {
            pl = PropertyList.add("ARG1>ARG2", 1.0, pl);
            //pl = PropertyList.add("ARG1>ARG2&"+category, 1.0, pl);
        }
        ...
        pl = PropertyList.add("DIST="+(connStart - arg1HeadPos)/2, 1.0, pl);
        //pl = PropertyList.add("DIST'="+(arg2HeadPos - arg1HeadPos)/2, 1.0, pl);
        
        //quotation feature                
        int arg1QuoteStart = findValue(leaves, arg1HeadPos-1, 0, "``");
        int arg1QuoteEnd = findValue(leaves, arg1HeadPos+1, leaves.size()-1, "''");
        int connQuoteStart = findValue(leaves, connStart-1, 0, "``");
        int connQuoteEnd = findValue(leaves, connEnd+1, leaves.size()-1, "''");
        
        if ((arg1QuoteStart != -1) && (arg1QuoteEnd != -1)) {
            pl = PropertyList.add("ARG1InQuote", 1.0, pl);
            if (connQuoteStart == arg1QuoteStart) {
                pl = PropertyList.add("ARG1&CONNInQuote", 1.0, pl);
            }
        }
        if (connQuoteStart != -1 && connQuoteEnd != -1) {
            pl = PropertyList.add("CONNInQuote", 1.0, pl);
        }
        return pl;
    }
