 ...
 */
public class PDTBArg1PosTrainer {
    ...
    public PDTBArg1PosTrainer(String pdtbRoot, String ptbRoot, String depRoot) {
                ...
                for (PDTBRelation relation : relations) {
                    if (relation.getType().equals("Explicit")) {
                        ...
                        if (/*sentential2 && sentential1 && */(lineNumber1 == lineNumber2)) {
                            addInstancesThroughPipe(relation, ptbTrees.get(lineNumber2), depGraphs.get(lineNumber2), instanceList, true);
                        } else {
                            addInstancesThroughPipe(relation, ptbTrees.get(lineNumber2), depGraphs.get(lineNumber2), instanceList, false);
                        }
    ...
    private void addInstancesThroughPipe(PDTBRelation relation, Tree root, SimpleDepGraph depGraph, InstanceList instanceList, boolean sentential) {        
        ...
        if (connHeadLeaves.isEmpty()) {
            System.out.println("Connective head leaves is empty!!!!");
        ...
        if (category.matches("Sub.*")) {
            if (!sentential) {
                System.out.println("NOT SENTENTIAL!!!");
                System.out.println(relation);
                root.pennPrint();
            }            
        }
                
        Arg1PosInstance instance = new Arg1PosInstance(tree, depGraph, connStart, connEnd, relation, sentential);
        instanceList.addThruPipe(instance);
    }
    
    private Pipe defaultPipe() {
        ArrayList<Pipe> pipes = new ArrayList<Pipe>();
        pipes.add(new Arg1PosInstance2FeatureVector());
    ...
    public static void main(String args[]) {
        PDTBArg1PosTrainer trainer = new PDTBArg1PosTrainer();
