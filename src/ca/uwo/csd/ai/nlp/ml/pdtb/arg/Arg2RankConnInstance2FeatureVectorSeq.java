 ...
 */
public class Arg2RankConnInstance2FeatureVectorSeq extends Pipe {
    ...
    public Arg2RankConnInstance2FeatureVectorSeq() {
        super(new Alphabet(), new LabelAlphabet());
        connAnalyzer = new ConnectiveAnalyzer();
    }
    
    public Arg2RankConnInstance2FeatureVectorSeq(String connLexiconPath) {
        super(new Alphabet(), new LabelAlphabet());
    ...
    public Instance pipe(Instance carrier) {
        Arg2RankConnInstance instance = (Arg2RankConnInstance) carrier;
        Tree root;
        try {
        ...
        List<Integer> headPositions = instance.getHeadPos();
        FeatureVector fvs[] = new FeatureVector[headPositions.size()];
        
        for (int i = 0; i < headPositions.size(); i++) {            
            int headPos = headPositions.get(i);
        ...
        carrier.setTarget(ldict.lookupLabel(String.valueOf(instance.getTrueHeadPos())));        
    ...
    private PropertyList addDependencyFeatures(PropertyList pl, Tree root, SimpleDepGraph depGraph, int connStart, int connEnd, int headPos) { 
        ...
        //added on 10-03-2012
        List<String> pathList = depGraph.getPathAsList(headPos, connStart, false);
        int limit = 10;
        String depPath = "";
        if (pathList != null) {
            for (int i = 0; i < limit && i < pathList.size(); i++) {
                depPath += pathList.get(i) + ":";
            }            
        } else {
            depPath = "NONE";
        }
        ...
        //added 10-03-2012
        //new head is from dependency treee, the word with (possible) no parent within arg2 scope
        //distance from the parent can be a good feature
        
        
        int parent = depGraph.getParent(headPos);
        if (parent == -1) {
            //pl = PropertyList.add("P-REL=NONE", 1.0, pl);
        } else {
            /*if ((connEnd <= parent && parent < headPos)) {                
                pl = PropertyList.add("HP=CPH", 1.0, pl);
            } else if (headPos < parent && parent <= connStart) {                
                pl = PropertyList.add("HP=HPC", 1.0, pl);
            } else if (connEnd < headPos && headPos < parent) {                
                pl = PropertyList.add("HP=CHP", 1.0, pl);
            } else if (parent < headPos && headPos < connStart) {                
                pl = PropertyList.add("HP=PHC", 1.0, pl);
            } else {                
                pl = PropertyList.add("HP=O", 1.0, pl);
            }*/
            Tree parentTree = root.getLeaves().get(parent).parent(root);
            Tree tagNode = root.getLeaves().get(headPos).parent(root);
            pl = PropertyList.add("P_POS="+parentTree.value()+"&POS="+tagNode.value(), 1.0, pl);                                    
        }
        return pl;
    }
