 ...
 */
public class PPIPipe2 extends Pipe{
    ...
    public Instance pipe(Instance carrier) {
        ...
        addKeyFeatures(featureMap, s, depGraph, ccDepGraph, entity1, entity2);
        ...
        carrier.setData(fv);
        //carrier.setData(getTokenSequence(featureMap));
        return carrier;
    }
    ...
    TokenSequence getTokenSequence(Map<String, String> featureMap) {
    ...
    void addSurfaceFeatures(Map<String, String> featureMap, Sentence s, SimpleDepGraph depGraph, int entity1, int entity2) {            
        ...
        int windowLen = 4;
        for (int i = entity1-windowLen; i < entity1; i++) {
            if (i >= 0) {
                if (s.get(i).getTag("DOMAIN") != null) {
                    featureMap.put("E1-L-WIN-" + s.get(i).word(), String.valueOf(true));
                }
            }            
        }
        for (int i = entity2 + 1; i < entity2 + windowLen; i++) {
            if (i < s.size()) {
                if (s.get(i).getTag("DOMAIN") != null) {
                    featureMap.put("E2-R-WIN-" + s.get(i).word(), String.valueOf(true));
                }
            }
        }
        ...
        for (int i = entity1 + 1; i < entity2; i++) {
            if (s.get(i).getTag("DOMAIN") != null) {
                featureMap.put("E12-M-WIN-" + s.get(i).word(), String.valueOf(true));                
            }
        }        
        
        featureMap.put("TOKEN_DIST", String.valueOf((entity2 - entity1)));
        ...
        if (entity1 == entity2) {
            ...
            if (entity2 < (s.size() - 1) && s.get(entity2 + 1).getTag("DOMAIN") != null) {
                featureMap.put("SAME-DOM", s.get(entity2 + 1).getTag("DOMAIN"));
            } else if (parent != -1 && s.get(parent).getTag("DOMAIN") != null) {
                featureMap.put("SAME-DOM", s.get(parent).getTag("DOMAIN"));
            } else {
                for (SimpleDependency sd : depGraph.getGovDependencies(entity1)) {
                    int dep = sd.dep();
                    if (s.get(dep).getTag("DOMAIN") != null) {
                        featureMap.put("SAME-DOM", s.get(dep).getTag("DOMAIN"));
                        break;
                    }
                }
            }       
    ...
    void addDependencyFeatures(Map<String, String> featureMap, Sentence s, SimpleDepGraph depGraph, SimpleDepGraph ccDepGraph, int entity1, int entity2) {
        ...
        if (pathAsRelnList == null) {
            featureMap.put("NULL_PATH", String.valueOf(entity2 - entity1));
            return;
        }/* else if (pathAsRelnList.size() >= 8) {
            featureMap.put("LONG_PATH", String.valueOf(true));
            return;
        }*/        
        ...
        for (SimpleDependency sd : pathAsRelnList) {            
            ...
            if (s.get(gov).getTag("DOMAIN") != null) {
                ...
                featureMap.put(reln+"%", s.get(gov).word());
                
                featureMap.put(s.get(gov).getTag("DOMAIN"), String.valueOf(true));
                if (gov < entity1) {
                    featureMap.put("L-" + s.get(gov).getTag("DOMAIN"), reln);
                } else if (gov < entity2) {
                    featureMap.put("M-" + s.get(gov).getTag("DOMAIN"), reln);
                } else {
                    featureMap.put("R-" + s.get(gov).getTag("DOMAIN"), reln);
                }
            }
            
            if (!ccDepGraph.getGovDependencies(gov, Pattern.compile("neg")).isEmpty()) {
                featureMap.put("NEG-" + reln, String.valueOf(true));
            }
            if (prev == gov) {
                cur = dep;
            } else {
            ...
            if (reln.matches("nsubj|nsubjpass")) {
                cpath.append("subj:");                
            } else if (reln.matches("rcmod|partmod")) {
                cpath.append("mod:");
            } else if (reln.matches("prep.*")) {
                cpath.append(reln + ":");
            } else {
                cpath.append("o:");
            }
        ...
        featureMap.put("DEP_PATH_LEN", String.valueOf(pathAsRelnList.size()));
        featureMap.put("CPATH", cpath.toString());        
    ...
    void addKeyFeatures(Map<String, String> featureMap, Sentence s, SimpleDepGraph depGraph, SimpleDepGraph ccDepGraph, int entity1, int entity2) {
        //find in the middle
        int key = -1;
        for (int i = entity1 + 1; i < entity2; i++) {
            if (s.get(i).getTag("DOMAIN") != null) {
                List<Integer> reachableIndices = ccDepGraph.getReachableIndices(i, true, 5);
        ...
        if (key == -1) {
            for (int i = entity1 + 1; i < entity2; i++) {
                List<Integer> reachableIndices = ccDepGraph.getReachableIndices(i, true, 5);
                if (reachableIndices.contains(entity1) && reachableIndices.contains(entity2)) {
                    for (SimpleDependency sd : ccDepGraph.getGovDependencies(i)) {
                        if (s.get(sd.dep()).getTag("DOMAIN") != null) {
                            key = sd.dep();
                            break;
                        }
                    }
        ...
        if (key != -1) {
            featureMap.put("KEY", s.get(key).word());
            if (key < entity1) {
                featureMap.put("L-KEY", String.valueOf(true));
            } else if (key < entity2) {
                featureMap.put("M-KEY", String.valueOf(true));
            } else {
                featureMap.put("R-KEY", String.valueOf(true));
            }
            
            featureMap.put("POS-KEY", s.get(key).getTag("POS"));
            /*if (s.get(key).getTag("POS").startsWith("V")) {
                List<Integer> reachableIndices = ccDepGraph.getReachableIndices(key, true, 5);
                boolean otherProtein = false;
                for (int r : reachableIndices) {
                    if (r != entity1 && r != entity2) {
                        if (s.get(r).word().contains("PROTEIN")) {
                            otherProtein = true;
                            break;
                        }
                    }
                }
                featureMap.put("OTHER-PROT", String.valueOf(otherProtein));
            }*/
        } else {
            featureMap.put("NO-KEY", String.valueOf(true));
        }
    }
